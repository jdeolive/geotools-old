...
/*		polygonAttribs.add(new AttributeTypeDefault("PEN_WIDTH", String.class));
	...
	private Feature readObject(BufferedReader mifReader, BufferedReader midReader) throws DataSourceException
	{
		...
		if (line==null)
			return null;
		if (line.substring(0, line.indexOf(' ')).equalsIgnoreCase(TYPE_POINT))
		{
			// Read point data
			System.out.println("Reading POINT");
			feature = readPointObject(mifReader, midReader);
		}
		else if (line.substring(0, line.indexOf(' ')).equalsIgnoreCase(TYPE_LINE))
		{
			// Read line data
			System.out.println("Reading LINE");
			feature = readLineObject(mifReader, midReader);
		}
		else if (line.substring(0, line.indexOf(' ')).equalsIgnoreCase(TYPE_PLINE))
		{
			// Read pline data
			System.out.println("Reading PLINE");
			feature = readPLineObject(mifReader, midReader);
		}
		else if (line.substring(0, line.indexOf(' ')).equalsIgnoreCase(TYPE_REGION))
		{
			// Read region data
...
//			midValues.putAll(shading);
			...
			System.out.println("Built point feature : "+x+" "+y);
		}
		catch(NumberFormatException nfexp)
		{
...
//			midValues.putAll(shading);
			...
			System.out.println("Built line feature : "+x1+" "+y1+" - "+x2+" "+y2);
		}
		catch(NumberFormatException nfexp)
		{
			throw new DataSourceException("Exception reading Point data from MIF file : "+nfexp.getMessage());
...
//			midValues.putAll(shading);
			...
			System.out.println("Read polyline ("+coords.size()+")");
		}
		catch(NumberFormatException nfexp)
		{
			throw new DataSourceException("Exception reading Point data from MIF file : "+nfexp.getMessage());
...
//			midValues.putAll(shading);
			...
			System.out.println("Read Region ("+polys.size()+")");
		}
		catch(NumberFormatException nfexp)
		{
			throw new DataSourceException("Exception reading Point data from MIF file : "+nfexp.getMessage());
	...
	private Hashtable readMid(BufferedReader midReader) throws DataSourceException
	{
		...
		try
		{
			...
			System.out.println("Read MID "+midLine);
		}
		catch(IOException ioexp)
		{
	...
	private Hashtable readShading(BufferedReader mifReader) throws IOException
	{
		...
		while (shadeType!=null && isShadingClause(shadeType))
		{
			...
			System.out.println("Read shading ("+name+")");
