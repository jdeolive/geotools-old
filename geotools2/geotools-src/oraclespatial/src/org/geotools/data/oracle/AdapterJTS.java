 ...
 * @version $Id: AdapterJTS.java,v 1.1 2003/07/08 15:50:48 cholmesny Exp $
 */
public class AdapterJTS implements GeometryAdapter {
    ...
    private final String FORMAT_NAME = "JTS";
    private final String FORMAT_VERSION = "1.3";
    private final Class[] SUPPORTED_OUTPUT = { com.vividsolutions.jts.geom.Geometry.class };
    private final Class[] SUPPORTED_INPUT = { com.vividsolutions.jts.geom.Geometry.class };
    private final Class[] SUPPORTED_PASS_THROUGH = {  };
    ...
    private com.vividsolutions.jts.geom.Geometry sdoToJts(Geometry geom)
        ...
        if (geometryType.equals(oracle.sdoapi.geom.Point.class)) {
            ...
            output = jtsFactory.createPoint(new Coordinate(point.getX(), point.getY(), point.getZ()));
            ...
            LinearRing exterior = jtsFactory.createLinearRing(curveStringToCoordinates(
    ...
    private Coordinate[] coordPointsToCoordinates(CoordPoint[] coordPoints, int nDim)
        throws InvalidGeometryException {
        if ((nDim != 3) && (nDim != 2)) {
            String message = "AdapterJTS.arrayToCoordinates(): nDim was not 2 or 3. nDim = " +
                nDim;
            LOGGER.warning(message);
            throw new InvalidGeometryException(message);
        }
     ...
     * @throws InvalidGeometryException
     *
     * @task REVISIT: Need to work out how to set the SRID properly. JTS stores the SRID as an
     *       integer. When converting from an SDO to a JTS we can just set this to the SRID from
     *       the SDO.  However, since SRIDs need to map to the SRID table in Oracle, when
     *       converting from a JTS we don't know if the SRID exists because it could have been set
     *       on the JTS geom anywhere.  At this stage, just hope the sdoFactory chooses a sensible
     *       default.
     */
    private oracle.sdoapi.geom.Geometry jtsToSdo(com.vividsolutions.jts.geom.Geometry geom)
        ...
        if (geometryType.equals(com.vividsolutions.jts.geom.Point.class)) {
        ...
        } else if (com.vividsolutions.jts.geom.GeometryCollection.class.isAssignableFrom(
                    geom.getClass())) {
            ...
            output = sdoFactory.createGeometryCollection((Geometry[]) sdoGeometries.toArray(
                        new oracle.sdoapi.geom.Geometry[0]));
        } else {
            String message = "Got a geometry that I don't know how to handle: " +
                geometryType.getName();
            LOGGER.warning(message);
