 ...
 *
 * @version $Id: WMSServer.java,v 1.2 2002/07/15 17:09:59 loxnard Exp $
...
public interface WMSServer
{
	 ...
	 * @param version The requested version of the Capabilities XML
         *        - used for version Negotiation.
	 * @return The capabilites of this server. The string must conform to
         *         the OGC WMS spec at
	 ...
	 * @param layers The Identifying names of the Layers to display.
         *        These Layers are assumed to be installed and ready on the
         *        server. The layers are drawn from the order in which they are
         *        in the list (first bottommost).
	 * @param styles The styles to use for each Layer - one for one.
	 * @param srs The Spatial Reference system to use.
	 * @param bbox The bounding box to use for the map. In the form
         *        minx, miny, maxx, maxy (lower left, upper right), in SRS
         *        units. The values are dependant on the spatial reference
         *        system in use, and may be rounded off.
	 ...
	 */
	public BufferedImage getMap(String [] layers, String [] styles, String srs, double [] bbox,
                                    int width, int height, boolean transparent, Color bgcolor)
                                    throws WMSException;
	
	/**
         * Gets the Feature info for given Layers. The first 5 parameters are
         * the same as those in the call to getMap. Feature Info can only be
         * returned for those layers for which the attribute queryable = 1.
	 * @param layers The Identifying names of the Layers to display.
         *        These Layers are assumed to be installed and ready on the
         *        server. The layers are drawn from the order in which they
         *        are in the list (first bottommost).
	 * @param srs The Spatial Reference system to use.
