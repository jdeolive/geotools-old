 ...
 * support SLD Servers. A full Capabilities object is returned by an
 * implementation of WMSServer to the WMSServlet, and translated into a valid
 * WMS Capabilities XML stream.  Implementors of WMSServer should construct
 * this and fill it with relevent information for use in the getCapabilities
 * call.
 */
public class Capabilities {
    ...
    /**
     * If a Vendor wants to place extra information betwene the
     * &lt;VendorSpecificInformation&gt; tags, it goes here (in XML format)
     */
    ...
    /**
     * Sets the Vendor-Specific capabilities for this server. This is purely
     * optional.
     *
     * @param vendorXML An XML string of any vendor specific capabilities this
     *        server supports.
     */
    ...
    /**
     * If Vendor-Specific capabilites have been set for this object, returns
     * it.
     *
     ...
     *
     * @throws CapabilitiesException DOCUMENT ME!
     */
    public void addLayer(String name, String title, String srs, double[] bbox)
        throws CapabilitiesException {
        // See if the layer exists already
        if (getLayer(name) != null) {
            throw new CapabilitiesException(CapabilitiesException.EXP_DUPLICATELAYER,
                "Layer already exists in call to addLayer");
        }
     ...
     * @throws CapabilitiesException DOCUMENT ME!
     */
    public void addLayer(String parent, String name, String title, String srs,
        double[] bbox) throws CapabilitiesException {
        // See if the layer exists already
        Layer lParent = getLayer(parent);

        if (lParent == null) {
            throw new CapabilitiesException(CapabilitiesException.EXP_INVALIDLAYER,
                "Parent Layer does not exist in call to addLayer");
        }
    ...
    /**
     * Adds a style for an existing layer. If a style with the given name is
     * already available to the given layer, then no style is added.
     *
     * @param layerName DOCUMENT ME!
     * @param styleName DOCUMENT ME!
     * @param styleTitle DOCUMENT ME!
     * @param legendUrl DOCUMENT ME!
     *
     * @throws CapabilitiesException DOCUMENT ME!
     */
    public void addStyle(String layerName, String styleName, String styleTitle,
        String legendUrl) throws CapabilitiesException {
        // Find the relevent layer
        Layer l = getLayer(layerName);

        if (l == null) {
            throw new CapabilitiesException(CapabilitiesException.EXP_INVALIDLAYER,
                "Invalid layer name given in call to addStyle");
        }
    ...
    /**
     * Gets the available styles for a given layer. This method is recursive
    ...
    private Vector getAvailableStyles(String layerName, Layer node) {
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            Vector childStyles = getAvailableStyles(layerName,
                    (Layer) children.elementAt(i));
