...
public class GtWmsServer implements WMSServer
{
	...
	LayerEntry [] layers;
	/** The loaded Theme objects - loaded according to the entries in the layers.xml file */
	Hashtable themes;
	/** The Viewer object to use for drawing */
	MiniViewer viewer;

	/** Specialized version number for sending back capabilities as a text string (not WMS compatible) */
	...
	public BufferedImage getMap(String [] layers, String [] styles, String srs, double [] bbox, int width, int height, boolean transparent, Color bgcolor) throws WMSException
	{
		...
		try
		{
			...
			viewer.setExtent(new GeoRectangle(bbox[0], bbox[1], bbox[2]-bbox[0], bbox[3]-bbox[1]));
	...
	/** This method is a slight hack. It's for returning a version of the capabilities xml, but in plain text, to allow for non-xml happy scripting languages like php.
