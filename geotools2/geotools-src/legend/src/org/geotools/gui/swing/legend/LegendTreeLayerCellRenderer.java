...
public class LegendTreeLayerCellRenderer extends javax.swing.JPanel
    ...
    /**
     * a array reference to the components on the panel convenient for setting
     * the same attribute
     */
    private JComponent[] renderChild = null;
    ...
    public LegendTreeLayerCellRenderer() {
        super();
        initComponents();
        renderChild = new JComponent[] { legendNoteCheckBox, treeNoteIconJLabel };
    }
    ...
    private void initComponents() {
        ...
        treeNoteIconJLabel.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
    ...
    public void setBackground(Color color) {
        ...
        for (int i = 0; i < renderChild.length; i++) {
            renderChild[i].setBackground(color);
        }
    }
    ...
    public java.awt.Component getTreeCellRendererComponent(
        ...
        // this select means the if the layer is selected whereas the selected as one of the input parameters
        // means if the note is selected in the tree
        setLayerSelected(userObject.isSelected());
