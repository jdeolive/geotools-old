...
/**
 * This is a starting point for providing your own FeatureStore
 * implementation.
 * 
...
public class JDBCFeatureStore extends JDBCFeatureSource implements FeatureStore {
                ...
                for( int i=0; i<type.length; i++){
                    try {
                        ...
                        throw new DataSourceException(
                            "Could not update feature "+feature.getID() + " with "+type[i].getName()+"="+value[i], e );
                    }
                }        
                writer.write();                                
            }
    ...
    public Set addFeatures(FeatureReader reader) throws IOException {
        ...
        Feature newFeature;
        FeatureWriter writer = getDataStore().getFeatureWriterAppend( typeName, getTransaction() );
        try {
            while( reader.hasNext() ){
                try {
                    feature = reader.next();
