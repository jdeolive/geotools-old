 ...
 *
 * $Id: ExtentMosaic.java,v 1.2 2002/07/13 08:56:36 loxnard Exp $
...
public class ExtentMosaic implements Extent {
    ...
    /**
     * The overlap mode - whether to split an Extent added to this mosaic
     * to fit into the parts around the edges (false), or split the mosaic
     * to fit around the newly added Extent (true).
     */
    ...
    /**
     * Constructor.
     * @param overlap If true, then the mosaic is applied in favour of any
     *        new Extents added.  That is, the last added Extent is kept
     *        whole, chopping the mosaic to fit around it. If false,
     *        the reverse applies, as the newest Extent is always
     ...
     * already added to the overall mosaic.  If the overlap mode is true
     * (the default), the mosaic is chopped to fit around the new Extent,
     * ex. If false, then the Extent ex is chopped to fit around the
     * mosaic. The return value is the same in both cases, which is the
     * result of chopping the new Extent around the mosaic.
     * @param ex The Extent to apply to the mosaic.
     * @return The difference between the added Extent and the mosaic
     *         already in place.
     */
    ...
    /**
     * Removes the oldest Extent from this ExtentMosaic.
     * Useful for memory-management routines.
    ...
    /**
     * Gets the Extents currently in this mosaic, in the order in which
     * they were added (oldest first).
     * @return The Extents currently held by the mosaic. The Extents will
     *         likely have been changed as they were applied to fit into
     *         the mosaic, and will probably not be the same Extents as
     *         those addded initially.
     */
    ...
    /**
     * Gets the Extents which make up the leftover space when ex is applied
     * to mosaic.
     * @param extents The Extents to patch into the mosaic of extents.
     * @param mosaic The List of Extents already in place.
     * @return The Extents which make up the difference between ex
     *         and mosaic.
     */
