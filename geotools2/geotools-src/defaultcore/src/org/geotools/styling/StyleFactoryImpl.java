...
public class StyleFactoryImpl extends StyleFactory {
    ...
    public Stroke createStroke(Expression color, Expression width,
        ...
        return createStroke(color, width, opacity,
            ...
            filterFactory.createLiteralExpression("square"), null,
    ...
    public RasterSymbolizer createRasterSymbolizer(
        ...
        if (geometryPropertyName != null) {
            rs.setGeometryPropertyName(geometryPropertyName);
        }

        if (opacity != null) {
            rs.setOpacity(opacity);
        }

        if (channel != null) {
            rs.setChannelSelection(channel);
        }

        if (overlap != null) {
            rs.setOverlap(overlap);
        }
        ...
        if (ce != null) {
            rs.setContrastEnhancement(ce);
        }

        if (relief != null) {
            rs.setShadedRelief(relief);
        }

        if (outline != null) {
            rs.setImageOutline(outline);
        }
    ...
    public ChannelSelection createChannelSelection(
        ...
        if ((channels != null) && (channels.length > 0)) {
            cs.setSelectedChannels(channels);
        }
