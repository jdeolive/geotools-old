...
public class Java2DRenderer implements org.geotools.renderer.Renderer {
    ...
    /** the size of the pathCache */
    private static final int pathCacheSize = 1000;
    ...
    private void renderPolygon(Feature feature, PolygonSymbolizer symbolizer) {
        if (LOGGER.isLoggable(Level.FINE)) {
            LOGGER.fine("rendering polygon with a scale of " + 
    ...
    private void renderString(Graphics2D graphic, double x, double y, double dx, 
    ...
    private void drawHalo(Halo halo, Graphics2D graphic, double x, double y, 
    ...
    private boolean fillDrawTextMark(Graphics2D graphic, 
                                     com.vividsolutions.jts.geom.Point point, 
                                     TextMark mark, int size, double rotation, 
                                     Feature feature) {
        return fillDrawTextMark(graphic, point.getX(), point.getY(), mark, size, 
                                rotation, feature);
    }
     ...
     */
    private void drawWithGraphicStroke(Graphics2D graphic, GeneralPath path, 
    ...
    private void addToPath(final GeneralPath path, final Coordinate[] coords) {
        ...
        //        double[] lastPointDrawn = new double[2];
        //        lastPointDrawn[0] = coords[0].x;
        //        lastPointDrawn[1] = coords[0].y;
        //        double pixel2 = this.scaleDenominator * this.scaleDenominator;
        for (int i = 1; i < coords.length; i++) {
            //            dx = (coords[i].x - lastPointDrawn[0]); 
            //            dy = (coords[i].y - lastPointDrawn[1]);
            //            d2 = dx*dx + dy*dy;
            //        
            //            if(d2>pixel2){ // if its less than a pixel ignore it.
            path.lineTo((float) coords[i].x, (float) coords[i].y);

            //                lastPointDrawn[0] = coords[i].x;
            //                lastPointDrawn[1] = coords[i].y;
            //            }
        }
    }
