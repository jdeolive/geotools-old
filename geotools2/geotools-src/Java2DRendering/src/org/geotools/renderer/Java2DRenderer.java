...
Renderer2D {
  ...
  protected WeakHashMap cache = new WeakHashMap();
  ...
  AffineTransform transform) {
    ...
    final Layer[] layers = context.getLayerList().getLayers();
    for (int l = 0; l < layers.length;l++) {
      if (!layers[l].getVisability()) {
      ...
      FeatureCollection fc = layers[l].getFeatures();
      ...
      //process them
      FeatureTypeStyle[] featureStylers = layers[l].getStyle().getFeatureTypeStyles();
      processStylers(fc, featureStylers, graphics);
      
      Date end = new Date();
      
      if (LOGGER.getLevel() == Level.INFO) { //change to fine when finished
        LOGGER.info("Time to render is " +
  ...
  final FeatureTypeStyle[] featureStylers) {
    // this code is garbage, but hey, its deprecated - IanS
    FeatureCollection fc = FeatureCollections.newCollection();
    fc.addAll(Arrays.asList(features));
    processStylers(fc, featureStylers, this.graphics);
  }
      ...
      while (features.hasNext()) {
        ...
        .getEnvelopeInternal())) {
          ...
          if (LOGGER.isLoggable(Level.FINER)) {
            LOGGER.finer("skipping " + feature.getID());
          }
        ...
        if (LOGGER.isLoggable(Level.FINE)) {
          LOGGER.fine("feature is " + typeName +
          " type styler is " + fts.getFeatureTypeName());
        }
        
        if (typeName.equalsIgnoreCase(fts.getFeatureTypeName())) {
          ...
          for (int k = 0; k < rules.length; k++) {
            ...
            !rules[k].hasElseFilter()) {
              ...
              (filter.contains(feature) == true)) {
                ...
                processSymbolizers(rendered,feature, symbolizers);
