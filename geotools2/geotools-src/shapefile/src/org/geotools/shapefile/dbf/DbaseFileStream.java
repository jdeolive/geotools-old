...
abstract class DbaseFileStream {
    ...
    public String getFieldName(int i) {
        return myHeader.getFieldName(i);
    }
    
    /**
     * Returns the number of fields in the dbf file
     */
    ...
    public boolean equiv(DbaseFileStream that) {
        ...
        if ((that == null) || (this.myHeader == null) || (that.myHeader == null)) {
        ...
        for (int i = 0; i < myHeader.getNumFields(); i++) {
            ...
            if (this.myHeader.getFieldType(i) != that.myHeader.getFieldType(i)) {
                ...
                System.err.println("Field " + getFieldNames()[i] + " has types of " +
                this.myHeader.getFieldType(i) + " and " + that.myHeader.getFieldType(i));
                return false;
            }
            
            if (this.myHeader.getFieldLength(i) != that.myHeader.getFieldLength(i)) {
                System.err.println("Failed DBF equivalence when comparing field lengths");
                System.err.println("Field " + getFieldNames()[i] + " has lengths of " +
                this.myHeader.getFieldLength(i) + " and " + that.myHeader.getFieldLength(i));
                return false;
            }
            
            if (this.myHeader.getFieldDecimalCount(i) != that.myHeader.getFieldDecimalCount(i)) {
                System.err.println("Failed DBF equivalence when comparing field decimal count");
                System.err.println("Field " + getFieldNames()[i] + " has decimal counts of " +
