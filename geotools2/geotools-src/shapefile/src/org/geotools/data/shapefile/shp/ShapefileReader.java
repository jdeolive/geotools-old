 ...
 * record will contain the bounds of the shape and will only read the shape
 * when the shape() method is called. This ShapefileReader.Record is the same
...
public class ShapefileReader {
	 ...
	 */
	public ShapefileReader(ReadableByteChannel channel, boolean strict)
	...
	private static int fill(ByteBuffer buffer, ReadableByteChannel channel)
		throws IOException {
	 ...
	 * presence of 8 more bytes, the length of a record. If this is true and
	 * the record indicates the next logical record number, there exists more
	 * records.
	 * 
	 * @throws IOException
	...
	public Record nextRecord() throws IOException {
		...
		if (recordType != ShapeType.NULL && recordType != fileShapeType) {
			throw new IllegalStateException(
				"ShapeType changed illegally from "
					+ fileShapeType
					+ " to "
					+ recordType);
		}
