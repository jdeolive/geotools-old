 ...
 * An instance of this interface should exist for all data sources which want to
 * take advantage of the dynamic datasource plugin system.
 *
 ...
 *
 * The factories are never called directly by users, instead the DataSourceFinder
 * class is used.
...
public interface DataSourceFactorySpi {
     ...
     * Typical key values for the map include:
     * url - location of a resource, used by file reading datasources.
     * dbtype - the type of the database to connect to, e.g. postgis, mysql
     * @throws DataSourceException Thrown if there were any problems creating or connecting the datasource.
     * @return The created DataSource, this may be null if the required resource was not found
     * or if insufficent parameters were given.
     ...
     * @return A human readable description that is suitable for inclusion in a list of
     * available datasources.
     */    
    public String getDescription();
    
    /** 
     * Test to see if this factory is suitable for processing the data pointed to by
     * the param tags.
     * If this datasource requires a number of parameters then this mehtod should check
     * that they are all present and that they are all valid.
     * If the datasource is a file reading data source then the extentions or mime
     * types of any files specified should be checked.
     * For example, a Shapefile datasource should check that the url param ends with
