...
public class TextStyleTest extends junit.framework.TestCase {
    ...
    private FeatureCollection buildFeatureCollection()
        throws Exception {
        com.vividsolutions.jts.geom.Envelope ex = new com.vividsolutions.jts.geom.Envelope(0, 50,
                0, 100);
        ...
        for (int j = 0; j < rows; j++) {
            ...
            for (int i = 0; i < points; i++) {
                ...
                org.geotools.feature.Feature pointFeature = pointType.create(new Object[] {
                            point, size, rotation
                        });
        ...
        org.geotools.feature.FeatureType lineType = org.geotools.feature.FeatureTypeFactory
            .newFeatureType(lineAttribute, "testLine");
        rows = 3;
        points = 3;
        ...
        for (int j = 0; j < rows; j++) {
            ...
            for (int i = 0; i < points; i++) {
                LineString line = makeSampleLineString(geomFac,
                        50 + ((i * ex.getWidth()) / points), j * 20, sign * 20, 20);
                Double size = new Double(12);
                Double poffset = new Double(off);
                sign--;

                org.geotools.feature.Feature lineFeature = lineType.create(new Object[] {
                            line, size, poffset
                        });
    ...
    public Style buildStyle() throws IllegalFilterException {
        ...
        // point style
        Mark triangle = sb.createMark(StyleBuilder.MARK_TRIANGLE,
        ...
        TextSymbolizer ts = sb.createTextSymbolizer(fill, new Font[] { font }, sb.createHalo(),
                sb.literalExpression("Point Label"), pp, null);
        style.addFeatureTypeStyle(sb.createFeatureTypeStyle("testPoint", new Symbolizer[] { ps, ts }));

        // line style
        Font font2 = sb.createFont("Lucida Sans", 10);
        font2.setFontSize(sb.attributeExpression("size"));

        TextSymbolizer ts2 = sb.createStaticTextSymbolizer(Color.decode("#FFAA00"), font2,
                "Line Label");
    ...
    private void performTestOnRenderer(Renderer2D renderer, String fileSuffix)
        ...
        if (INTERACTIVE) {
            java.awt.Frame frame = new java.awt.Frame("Text style test ("
        ...
        java.io.File file = new java.io.File(base.getPath(),
                "TextStyleTest_" + renderer.getClass().getName().replace('.', '_') + "_"
                + fileSuffix + ".png");
    ...
    private LineString makeSampleLineString(final GeometryFactory geomFac, double xstart,
