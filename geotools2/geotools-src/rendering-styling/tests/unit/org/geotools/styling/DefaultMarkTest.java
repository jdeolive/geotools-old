...
public class DefaultMarkTest extends junit.framework.TestCase {
     ...
     * Test lite renderer
     */
    public void testLiteRenderer() throws Exception {
        Context ctx = buildContext();
        performTestOnRenderer(new LiteRenderer(ctx));
    }

    /**
     * Test j2d renderer
     */
    public void testJ2DRenderer() throws Exception {
        Context ctx = buildContext();
        StyledRenderer sr = new StyledRenderer(null);
        sr.setContext(ctx);
        performTestOnRenderer(sr);
    }

    /**
     * Perform test on the passed renderer, which must be already configured with its context
     */
    private void performTestOnRenderer(Renderer2D renderer)
        throws Exception {
        java.net.URL base = getClass().getResource("testData/");
