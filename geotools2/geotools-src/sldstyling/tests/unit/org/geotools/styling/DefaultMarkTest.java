...
public class DefaultMarkTest extends junit.framework.TestCase {
    ...
    public void testSimpleRender() throws Exception {
        ...
        // Request extent
        com.vividsolutions.jts.geom.Envelope ex = new com.vividsolutions.jts.geom.Envelope(0,
                45, 0, 45);
        ...
        pointAttribute[2] = AttributeTypeFactory.newAttributeType("rotation",
                Double.class);
        pointAttribute[3] = AttributeTypeFactory.newAttributeType("name",
                String.class);
        ...
        labelAttribute[0] = AttributeTypeFactory.newAttributeType("centre",
                Point.class);
        labelAttribute[1] = AttributeTypeFactory.newAttributeType("name",
                String.class);
        labelAttribute[2] = AttributeTypeFactory.newAttributeType("X",
                Double.class);
        labelAttribute[3] = AttributeTypeFactory.newAttributeType("Y",
                Double.class);
        ...
        String[] marks = {
            "Circle", "Triangle", "Cross", "Star", "X", "Square", "Arrow"
        };
        double size = 6;
        double rotation = 0;
        int rows = 7;
        ...
        for (int j = 0; j < rows; j++) {
            ...
            org.geotools.feature.Feature pointFeature = labelType.create(new Object[] {
                        point, "" + size + "/" + rotation, new Double(0.3),
                        new Double(.5)
                    });
            features.add(pointFeature);

            for (int i = 0; i < marks.length; i++) {
                point = makeSamplePoint(geomFac, ((double) i * 5.0) + 10.0,
                        5.0 + (j * 5));
        ...
        for (int i = 0; i < marks.length; i++) {
            Point point = makeSamplePoint(geomFac, ((double) i * 5.0) + 10.0,
                    5.0 + (rows * 5));
            org.geotools.feature.Feature pointFeature = labelType.create(new Object[] {
        ...
        int w = 400;
        int h = 400;
        java.awt.image.BufferedImage image = new java.awt.image.BufferedImage(w,
                h, java.awt.image.BufferedImage.TYPE_INT_RGB);
        ...
        RenderedImage image2 = (RenderedImage) JAI.create("fileload",
                file2.toString());
            ...
            for (int y = 0; y < data.getWidth(); y++) {
                ...
                if ((notBlack(pixel1)) && (notBlack(pixel2))) { //Since text is black and fonts are not stable across platforms, ignore pixels where at least one is black.
                    ...
                    for (int band = 0; band < data2.getNumBands(); band++) {
                        assertEquals("mismatch in image comparison at (x: " + x
                            + " y: " + y + " band: " + band + ")",
                            pixel1[band], pixel2[band]);
                    }
                }
            }
        }
    ...
    private Point makeSamplePoint(
        final com.vividsolutions.jts.geom.GeometryFactory geomFac, double x,
        double y) {
        com.vividsolutions.jts.geom.Coordinate c = new com.vividsolutions.jts.geom.Coordinate(x,
                y);
