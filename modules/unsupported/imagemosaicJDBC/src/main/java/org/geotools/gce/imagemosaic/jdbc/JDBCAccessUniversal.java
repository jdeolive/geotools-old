...
class JDBCAccessUniversal extends JDBCAccessBase {
    ...
    private void initStatementStrings(Config config) {
        ...
        //	ymin < ymax0 );
        String whereClause = "";
        whereClause += (" ? > s." + config.getTileMinXAttribute() + " AND "); // param xmax
        whereClause += (" ? > s." + config.getTileMinYAttribute() + " AND "); // param ymax
        whereClause += (" ? < s." + config.getTileMaxXAttribute() + " AND "); // param xmin
        whereClause += (" ? < s." + config.getTileMaxYAttribute()); // param ymin
