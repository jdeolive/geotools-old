...
public class XMLConfigDigester {
    ...
    /**
     * Parses a complex datastore configuration file in
     * xml format into a {@link ComplexDataStoreDTO}
     *
     * @param dataStoreConfigUrl config file location
     *
     * @return a DTO object representing the datastore's
     * configuration
     *
     * @throws IOException if an error occurs parsing the file
     */
    public ComplexDataStoreDTO  parse(URL dataStoreConfigUrl)
        throws IOException {
     ...
     */
    private ComplexDataStoreDTO digest(final URL dataStoreConfigUrl)
        throws IOException {
        ...
        if (configStream == null) {
            throw new IOException("Can't open datastore config file "
                + dataStoreConfigUrl);
        }
