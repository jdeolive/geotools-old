 ...
 * @version $Id$
 * @source $URL:
 *         http://svn.geotools.org/geotools/branches/2.4.x/modules/unsupported/community-schemas/community-schema-ds/src/main/java/org/geotools/data/complex/ComplexDataStore.java $
 * @since 2.4
 */
public class ComplexDataStore /* extends AbstractDataStore */implements FeatureAccess {
    ...
    protected int getCount(final Query targetQuery) throws IOException {
        ...
        if (mapping.getGroupByAttNames().size() == 0) {

            ((DefaultQuery) unmappedQuery).setMaxFeatures(targetQuery.getMaxFeatures());
            count = mappedSource.getCount(unmappedQuery);

        } else {
            DefaultQuery groupAttsQuery = new DefaultQuery(unmappedQuery);
            groupAttsQuery.setPropertyNames(mapping.getGroupByAttNames());
            groupAttsQuery.setMaxFeatures(targetQuery.getMaxFeatures());
            FeatureCollection features = mappedSource.getFeatures(groupAttsQuery);
            Iterator iterator = features.iterator();
            count = 0;
            Feature feature = null;
            Object []startingGroupValues = new Object[features.getSchema().getAttributeCount()];
            Object []currFeatureAtts = new  Object[features.getSchema().getAttributeCount()];
            boolean breakGroup = true;
            try {
                while (iterator.hasNext()) {
                    feature = (Feature) iterator.next();
                    currFeatureAtts = feature.getAttributes(currFeatureAtts);
                    if(!Arrays.equals(startingGroupValues, currFeatureAtts)){
                        count++;
                        startingGroupValues = feature.getAttributes(startingGroupValues);
                    }
                }
            } finally {
    ...
    private List getSurrogatePropertyNames(String[] mappingProperties, FeatureTypeMapping mapping) {
        ...
        mappedType = (FeatureType) targetDescriptor.type();
