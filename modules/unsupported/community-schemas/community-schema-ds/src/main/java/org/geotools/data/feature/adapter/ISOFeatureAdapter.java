 ...
 * @version $Id$
 * @source $URL:
 *         http://svn.geotools.org/geotools/branches/2.4.x/modules/unsupported/community-schemas/community-schema-ds/src/main/java/org/geotools/data/feature/adapter/ISOFeatureAdapter.java $
 * @since 2.4
 */
public class ISOFeatureAdapter implements Feature, SimpleFeature {
    ...
    public List get(Name name) {
        Object value = getValue(name.getLocalPart());
        if (value == null) {
    ...
    public void setDefaultGeometryValue(Object geometry) {
        try {
            adaptee.setDefaultGeometry((Geometry) geometry);
        } catch (IllegalAttributeException e) {
            throw (RuntimeException) new RuntimeException().initCause(e);
        }
    }
    ...
    public void setValues(List values) {
        int attCount = adaptee.getNumberOfAttributes();
        Object[] contents = new Object[attCount];
        for (int i = 0; i < attCount; i++) {
            contents[i] = values.get(i);
        }
        setValues(contents);
    }
    ...
    public void setValues(Object[] values) {
        for (int i = 0; i < values.length; i++) {
            try {
                adaptee.setAttribute(i, values[i]);
            } catch (IllegalAttributeException e) {
                throw (RuntimeException) new RuntimeException().initCause(e);
            }
        }
