 ...
 */
package org.geotools.ws.protocol;
 ...
 * @since 2.5.x
 * @source $URL:
 *         http://svn.geotools.org/geotools/trunk/gt/modules/unsupported/app-schema/webservice/src/main/java/org/geotools/wfs/io/WFSConnectionFactory.java $
 * @deprecated use {@link WSProtocol}
 */
public abstract class WSProtocolHandler {
    ...
    public WSProtocolHandler(final Version version, final ConnectionFactory connectionFac) {
     ...
     * @return
     */
    public abstract boolean supports(HttpMethod method);
     ...
     *             service
     * @see #supports(WSOperationType, HttpMethod)
     */
    public abstract URL getOperationURL(HttpMethod method)
            throws UnsupportedOperationException;
    ...
    public HttpURLConnection createDescribeFeatureTypeConnection(final String typeName,
        ...
        if (HttpMethod.POST == method) {
            query = getOperationURL(HttpMethod.POST);
        } else {
    ...
    public URL getDescribeFeatureTypeURLGet(final String typeName) throws MalformedURLException {
        URL getUrl = getOperationURL(GET);
