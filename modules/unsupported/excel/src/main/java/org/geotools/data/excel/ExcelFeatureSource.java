 ...
 *
 */
public class ExcelFeatureSource extends ContentFeatureSource implements
		SimpleFeatureSource {
	...
	public ExcelFeatureSource(ContentEntry entry, Query query) {
		...
		int latCol = dataStore.getLatColumnIndex();
		int lonCol = dataStore.getLonColumnIndex();
		...
		for (int i = start; i < rows; i++) {
			...
			for (int col = data.getFirstCellNum(); col < data.getLastCellNum(); col++) {
				...
				if (col == latCol) {
					...
					// cast and handle
					final String name = header.getCell(col).getStringCellValue()
							.trim();
	...
	/**
	 * regenerate the filteredFeatures list if the query has changed
